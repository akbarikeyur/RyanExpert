...
func showAlertWithOption(_ title:String, message:String, btns:[String] ,completionConfirm: @escaping () -> Void,completionCancel: @escaping () -> Void){
    ...
    SweetAlert().showAlert(getTranslate(title), subTitle: message, style: .warning, buttonTitle: getTranslate(btns[0]), buttonColor: GreenColor, otherButtonTitle: getTranslate(btns[1])) { (isOtherButton) in
        if isOtherButton {
            completionCancel()
...
func showAlert(_ title:String, message:String, completion: @escaping () -> Void) {
    ...
    SweetAlert().showAlert(getTranslate(title), subTitle: getTranslate(message), style: .none, buttonTitle: "OK", buttonColor: GreenColor) { (isDone) in
        completion()
    }
}
